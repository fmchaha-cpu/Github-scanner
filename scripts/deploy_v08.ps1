...
Write-Host "=== Genshin Market Tracker v0.8 deploy ===" -ForegroundColor Cyan
...
Write-Host "6) Deploying Worker v0.8..." -ForegroundColor Yellow
npx wrangler deploy
...
for ($i = 1; $i -le 5; $i++) {
    try {
        ...
        if ($Health.version -eq "0.8") { break }
    } catch {
        if ($i -eq 5) { throw }
    }
    Start-Sleep -Seconds 2
}
if ($null -eq $Health -or $Health.version -ne "0.8") {
    $Got = if ($null -eq $Health) { "no response" } else { [string]$Health.version }
    throw "Worker verification failed. Expected version 0.8, got: $Got"
}
Write-Host ("Worker v0.8 confirmed. Historical records currently visible: " + $Health.historical_count) -ForegroundColor Green
Write-Host "v0.8 deployed. Existing D1 data and INGEST_TOKEN remain unchanged." -ForegroundColor Green
Write-Host "Next: run 'Import tracker historical seed' once, then start a fresh Genshin market scan." -ForegroundColor Green
